@echo off
:: This script:
::    - should be called as getReturnRequest.sh
...
set tempfile=%HF_WORKDIR%\R%hour%%min%%secs%%RANDOM%.txt
if not exist "%HF_WORKDIR%" (
    set tempfile=%scriptDir%\R%hour%%min%%secs%%RANDOM%.txt
)
...
%_python% "%homeDir%\scripts\Main.py" --getReturnRequests "%homeDir%" "%2%"
